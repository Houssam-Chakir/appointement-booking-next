        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
            ...
            <div className='bg-card rounded-lg border shadow-sm p-6 sticky top-20'>
              <div className='text-center mb-6'>
                <Avatar className='h-32 w-32 mx-auto mb-4'>
                  ...
                  <AvatarFallback className='bg-blue-100 text-blue-600 text-3xl'>{getInitials(provider.name)}</AvatarFallback>
                </Avatar>
          ...
          <div className='lg:col-span-2'>
            <div className='bg-card rounded-lg border shadow-sm p-6'>
              ...
              {/* User's Bookings for Selected Date */}
              ...
              {/* Available Slots */}
              <div className='mb-6'>
                <h3 className='font-semibold mb-4'>Available Time Slots ({availableSlots.length} available)</h3>

                {slots.length === 0 ? (
                  <p className='text-gray-500 text-center py-8'>Select a date to see available slots</p>
                ) : availableSlots.length === 0 ? (
                  <p className='text-gray-500 text-center py-8'>No available slots for the selected date and duration</p>
                ) : (
              ...
              {bookingResult && (
                <div
                  className={`px-4 py-3 rounded mb-4 ${
                    bookingResult.success ? "bg-green-50 border border-green-200 text-green-800" : "bg-red-50 border border-red-200 text-red-800"
                  }`}
                >
                  <p className='font-semibold'>{bookingResult.message}</p>
                  {bookingResult.success && <p className='text-sm mt-1'>Your appointment has been confirmed. Booking ID: {bookingResult.booking_group_id}</p>}
                </div>
              )}

              {/* Book Button */}
              <Button onClick={handleBooking} disabled={!selectedSlot || bookingLoading} className='w-full' size='lg'>
              ...
              <div className='mt-6 pt-6 border-t'>
                <p className='text-sm font-semibold mb-3 text-muted-foreground'>Slot Legend</p>
                <div className='flex flex-wrap gap-4 text-sm'>
                  <div className='flex items-center gap-2'>
                    <div className='w-5 h-5 bg-background border border-border rounded-md'></div>
                  ...
                  <div className='flex items-center gap-2'>
                    <div className='w-5 h-5 bg-green-50 border border-green-200 rounded-md'></div>
                    <span className='text-muted-foreground'>Your booking</span>
                  </div>
                  <div className='flex items-center gap-2'>
                    <div className='w-5 h-5 bg-muted rounded-md opacity-60'></div>
                    <span className='text-muted-foreground'>Unavailable</span>
                  </div>
                </div>
              </div>
              {userAppointments.length > 0 && (
                <div className='mb-6 p-4 mt-12 bg-green-50 border border-green-200 rounded-lg'>
                  <h3 className='font-semibold mb-3 text-green-900'>Your Bookings for {selectedDate.toLocaleDateString()}</h3>
                  <div className='space-y-3'>
                    {userAppointments.map((apt) => (
                      <div key={apt.id} className='flex items-center justify-between bg-white p-3 rounded-md border border-green-200'>
                        <div className='flex items-center gap-3'>
                          <Clock className='h-4 w-4 text-green-600' />
                          <div>
                            <p className='font-medium text-sm'>{apt.time_slot.slice(0, 5)}</p>
                            <p className='text-xs text-muted-foreground'>Booking ID: {apt.booking_group_id}</p>
                          </div>
                        </div>
                        <Button
                          variant='outline'
                          size='sm'
                          onClick={() => handleCancelAppointment(apt.booking_group_id)}
